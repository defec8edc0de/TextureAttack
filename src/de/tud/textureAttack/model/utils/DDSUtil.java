...
public class DDSUtil {
	...
	@Deprecated
	public static BufferedImage decompressTexture(final File file)
			throws IOException {
		return read(file);
	}

	/**
	 * Create a {@link BufferedImage} from a DXT-compressed dds-texture
	...
	public static BufferedImage loadBufferedImageFromByteBuffer(
			ByteBuffer data, int width, int height, DDSImage ddsimage) {
	 ...
	 * @throws UnsupportedDataTypeException
	 */
	public static BufferedImage loadBufferedImage(final TEXImage image)
			throws UnsupportedDataTypeException {
		return decompressTexture(image.getEmbeddedMaps(0).getMipMap(0)
				.getData(), image.getWidth(), image.getHeight(),
	...
	public static BufferedImage decompressTexture(final byte[] compressedData,
		...
		return new DXTBufferDecompressor(compressedData, width, height,
	 ...
	 * @return
	 */
	public static BufferedImage decompressTexture(
			final ByteBuffer textureBuffer, final int width, final int height,
			final Squish.CompressionType compressionType) {
		return new DXTBufferDecompressor(textureBuffer, width, height,
				compressionType).getImage();
	}
	...
	public static BufferedImage decompressTexture(
		...
		Squish.CompressionType compressionType = PixelFormats
				.getSquishCompressionFormat(pixelformat);
		return new DXTBufferDecompressor(textureBuffer, width, height,
				compressionType).getImage();
	}
	...
	public static void write(final File destinationfile,
			BufferedImage sourceImage, final int pixelformat,
		...
		if (!sourceImage.getColorModel().hasAlpha())
			sourceImage = ImageUtils.convert(sourceImage,
					BufferedImage.TYPE_4BYTE_ABGR);

		TextureMap maps = TextureFactory.createTextureMap(generateMipMaps,
				sourceImage);
	 ...
	 * 
	 */
	public void write(final File file, final TextureMap map,
			final int pixelformat) throws IOException {
		writeDDSImage(file, map.getDXTCompressedBuffer(pixelformat),
				map.getWidth(), map.getHeight(), pixelformat);
	}

	private static DDSImage writeDDSImage(final File file,
			final ByteBuffer[] mipmapBuffer, final int width, final int height,
			final int pixelformat) throws IllegalArgumentException, IOException {

		DDSImage writedds = DDSImage.createFromData(pixelformat, width, height,
				mipmapBuffer);
		// writedds.debugPrint();
		writedds.write(file);
		return writedds;
	...
	/**
	 * Returns the PixelFormat of a {@link File} This makes file-IO, therefor
	 * handle with caution!
	 * 
	 * @param file
	 * @return
	...
	private static Squish.CompressionType findCompressionFormat(
			DDSImage ddsimage) throws UnsupportedDataTypeException {
		int pixelFormat = ddsimage.getPixelFormat();
		return PixelFormats.getSquishCompressionFormat(pixelFormat);
	}

	/**
	 * Returns true for file formats supported by this library. Currently TEX
	 * and DDS reading is supported.
	 * 
	 * @param file
	 * @return
	 */
